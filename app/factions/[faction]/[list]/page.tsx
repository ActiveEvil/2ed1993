...
const buildEntry = (entry: RawEntry): Entry => {
...
export default async function Page(props: {
  ...
  if (list) {
    ...
    const jumpItems = list.unit_categories.map(({ category }) => ({
        ...
        <main id="main" className="flex flex-col items-center gap-4 w-full">
          ...
          <Panel className="flex flex-col gap-4 w-full max-w-5xl pb-4 md:pb-8">
            <section className="flex flex-col gap-8">
              ...
              {list.unit_categories.length ? (
                list.unit_categories.map((section) => {
                  const entries = section.army_list_entries.map(buildEntry);
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1 border-b-4 border-black pb-1 pmxx-4 md:mx-8">
                        <h3
                          id={generateAnchorId(section.category)}
                          className="font-title text-2xl uppercase tracking-wide"
                        >
                          {section.category}
                        </h3>
                        {section.note && (
                          <p className="max-w-[40ch] text-sm italic">
                            {section.note}
                          </p>
                        )}
                      </div>
                      {entries.length ? (
                        <div className="flex flex-col gap-6">
                          {groupBlocks(entries).map((block) => (
                            ...
                            >
                              {block.note && (
                                <p className="max-w-[70ch] text-sm italic">
                                  {block.note} hello!!!!
                                </p>
                              )}
              ...
              <section className="flex flex-col gap-8">
                <div className="mt-4 px-4 md:px-8">
                  <div className="relative flex flex-col items-center justify-center gap-4 w-full">
                    ...
                    <h2
                      id="Equipment"
                      className="md:px-2 bg-background font-title text-3xl text-center uppercase"
                    >
                      Equipment
                    </h2>
                  </div>
